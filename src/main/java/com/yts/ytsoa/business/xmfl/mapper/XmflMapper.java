...
public interface XmflMapper {
    ...
    void add(@Param("model") XmflModel model) throws Exception;
    ...
    void updateById(@Param("model") XmflModel model) throws Exception;

    @SelectProvider(type = XmflSql.class, method = "findAllSql")
    List<XmflModel> findAll(@Param("model") XmflModel model) throws Exception;

    @Select({
            "select * from" + table + " where uuid = #{id}"
    })
    XmflModel getById(@Param("id") String id) throws Exception;

    @Select({
            "select * from" + table + " where xmflmc = #{xmflmc}"
    })
    List<XmflModel> findByxmflmc(@Param("xmflmc") String xmflmc) throws Exception;
